    ...
    public static class Quiz
    {
        ...
        public static int[] GetEvenNumbers(int exclusiveUpperLimit)
        {
            return exclusiveUpperLimit <= 0
                ...
                : Enumerable.Range(1, exclusiveUpperLimit - 1).Where(val => val % 2 == 0).ToArray();
        }
        ...
        public static int[] GetSquares(int exclusiveUpperLimit)
        {
            return Math.Pow(exclusiveUpperLimit, 2) > int.MaxValue
                ...
                : Enumerable.Range(1, exclusiveUpperLimit < 0 ? 1 : exclusiveUpperLimit - 1).Where(val => val % 7 == 0)
                    .Select(i => (int) Math.Pow(i, 2)).OrderByDescending(x => x).ToArray();
        }
        ...
        public static FamilySummary[] GetFamilyStatistic(IReadOnlyCollection<IFamily> families)
        {
            return families == null
                ...
                {
                    FamilyID = f.ID, NumberOfFamilyMembers = f.Persons.Count,
                    AverageAge = f.Persons.Count == 0 ? 0 : f.Persons.Average(p => p.Age)
                }).ToArray();
        }
        ...
        {
            return text.ToUpper().Replace(" ", "").Where(c => char.IsLetter(c)).ToArray()
                .GroupBy(a => a).Select(c => (c.Key, c.Count())
                ).ToArray();
        }
    }
}
